...
shared editor stress probe. Tagged releases also attempt a clearly named experimental Linux x64
native tarball. Read its measured limitations before using it.
...
Linux x64 releases also attempt an `Editora-<version>-linux-x64-native-experimental.tar.gz`.
Extract it and run `./run-editora-native`; its launcher uses separate settings. The native build
