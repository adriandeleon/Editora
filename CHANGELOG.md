...
### Added
- **Save as Administrator (Linux).** Editing a file you lack write permission for (e.g. one under `/etc`) now offers to save it via the OS's own authentication prompt — Editora never sees the password; `pkexec`/polkit handles it. The View-mode banner on a read-only-on-disk file shows **Edit as Administrator**, and `Ctrl-S` (or the `File: Save as Administrator` palette command) then writes through the elevated helper, which rewrites the file in place so its owner and permissions are preserved. Off by default (Settings → Editor → Saving; palette `Toggle Save as Administrator`), Linux-only, requires `pkexec`; each elevated save prompts for authentication.
