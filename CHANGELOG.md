...
- **Internal: extracting `LspCoordinator` from `MainController` (phases 1–2).** The LSP navigation/format flows (go-to-definition, find-references, hover, document formatting + the hover popup) **and** the diagnostics routing (the per-open-file `problems` map, the `ProblemsPanel`, and the `publishDiagnostics` callback) moved into a new `CoordinatorHost`-style coordinator. The `LspManager` stays owned by `MainController` (Debug layers on the jdtls session, the MCP bridge reads its diagnostics) and is passed to the coordinator. No behavior change — the gating/lifecycle wiring follows in a later phase.
