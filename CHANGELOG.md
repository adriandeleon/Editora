...
- **Internal: began extracting `LspCoordinator` from `MainController`.** The LSP navigation/format flows (go-to-definition, find-references, hover, and document formatting — the `lsp.gotoDefinition`/`findReferences`/`hover`/`formatDocument` commands and editor right-click items, plus the dismissable hover popup) moved into a new `CoordinatorHost`-style coordinator. The `LspManager` stays owned by `MainController` (Debug layers on the jdtls session, the MCP bridge reads its diagnostics) and is passed to the coordinator. No behavior change — first slice of the LSP decomposition.
