 ...
 * else explicit {@code -s}; literal (non-regex) → {@code -F}; whole-word → {@code -w}; and {@code --no-ignore}
 * only when {@code respectIgnore} is false (rg honors {@code .gitignore}/hidden/binary by default).
 */
public final class RipgrepArgs {

    private RipgrepArgs() {}

    public static List<String> build(SearchQuery q, boolean respectIgnore, long maxFileBytes) {
